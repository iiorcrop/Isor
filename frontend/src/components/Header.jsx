    ...
    <div className="bg-white py-6 px-8 border-b border-gray-100">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-center gap-12 text-center md:text-left">
        ...
        <div className="flex flex-col items-center md:items-start max-w-2xl">
          <h1 className="text-[32px] font-bold text-[#1e703c] leading-tight font-serif tracking-tight">{data.title}</h1>
          <p className="text-[14px] text-gray-500 font-medium leading-relaxed mt-1">{data.subTitle}</p>
          <p className="text-[15px] text-[#b47c1c] font-bold mt-2 uppercase tracking-tight italic">{data.tagline}</p>
        </div>
        ...
        <div className="flex flex-col items-center md:items-end text-center md:text-right gap-4 border-t md:border-t-0 md:border-l border-gray-100 pt-6 md:pt-0 md:pl-12">
          <div>
            <p className="text-[#1e703c] font-bold text-[15px]">{data.affiliationText}</p>
            <p className="text-gray-500 text-[13px] font-medium">{data.estdText}</p>
          </div>

          <div>
            <p className="text-[#1e703c] font-bold text-[15px]">{data.journalTitle}</p>
            <p className="text-[#b47c1c] text-[13px] font-bold italic">{data.journalScore}</p>
          </div>
        </div>
      </div>
    </div>

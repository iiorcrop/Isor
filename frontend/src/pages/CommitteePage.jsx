...
const CommitteePage = () => {
  ...
  const fetchMembers = async () => {
    ...
    try {
      ...
      const uniquePeriods = [...new Set(res.data.map((m) => m.period).filter(Boolean))].sort((a, b) =>
        b.localeCompare(a),
      );
  ...
  // Filter members for selected period (or show all if ALL is selected)
  const filteredMembers = selectedPeriod === "ALL" 
    ? members 
    : (selectedPeriod ? members.filter((m) => m.period === selectedPeriod) : members);
        ...
        <header className="text-center mb-12 space-y-4">
          ...
          {periods.length > 1 && (
            <div className="flex flex-col items-center gap-4 mt-8">
              <label className="text-[#b47c1c] text-xs font-bold uppercase tracking-widest">
                Select Committee Period
              </label>
              <select
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#064e3b] text-white">
                      ...
                      <th className="px-6 py-5 text-sm font-bold uppercase tracking-wider">Tenure Period</th>
                    ...
                    <AnimatePresence mode="popLayout">
                      {filteredMembers.map((member, idx) => (
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <div className="font-bold text-[#064e3b] group-hover:text-[#b47c1c] transition-colors text-base">
                              {member.name}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                          ...
                          <td className="px-6 py-4">
                            <span className="bg-[#064e3b]/10 text-[#064e3b] text-xs font-bold px-3 py-1 rounded-full border border-[#064e3b]/20">
                              {member.period || "-"}
                            </span>
                          </td>
                          <td className="px-6 py-4 text-gray-600 text-sm italic leading-relaxed">
                            {member.organization || "-"}
                          </td>
                        </motion.tr>
                      ))}
